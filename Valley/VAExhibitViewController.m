...
#import "VAExhibitViewController.h"
#import "VADiscoverTableViewCell.h"
#import "VAExhibitSubject.h"
#import "VAUser.h"

#import "VAUserViewController.h"
#import "VAArticleViewController.h"

@interface VAExhibitViewController () <UIImageViewDelegate>

@end

@implementation VAExhibitViewController
...
- (void)loadData {
	...
	[self acquireDataWithURLString:EXHIBIT_URL_STRING parameters:nil success:^(NSDictionary *responseObject) {
		
		for (NSDictionary *subjectDir in responseObject[@"list"]) {
			[MagicalRecord saveWithBlock:^(NSManagedObjectContext *localContext) {
				[VAExhibitSubject MR_importFromObject:subjectDir inContext:localContext];
...
- (NSFetchRequest *)fetchRequest {
    ...
    NSFetchRequest *fetchRequest = [VAExhibitSubject MR_requestAllSortedBy:@"mtime" ascending:YES];
...
- (void)tableView:(UITableView *)tableView didSelectRowAtIndexPath:(NSIndexPath *)indexPath {
	...
	VAExhibitSubject *subject = [self.fetchedResultsController objectAtIndexPath:indexPath];
